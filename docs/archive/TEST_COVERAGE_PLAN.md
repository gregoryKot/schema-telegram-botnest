...
## Статус 2026-07-16 (финал, после волн 4-5)

Все этапы закрыты. Бэкенд: **1149 юнит + 31 e2e, 60.9% строк / 50.4% веток**
(было 353 / 23%); полы: src/auth 49.5%, src/utils 95.4%. webapp 234 (было 64),
miniapp 82 (было 0). Исправлено с регрессионными тестами: кризисная детекция
(7 пропусков), двойное шифрование encrypt-migration, FAIL при расхождении
суммы платежа, аудит выдачи роли, незакрытый startup-таймер, потеря данных
в автосейве кабинета терапевта (stale closure), доступ терапевта к mode-maps
после разрыва связи (+403 вместо 500), таймзонный сдвиг слотов западнее UTC,
500→400 на числовом 2FA-коде (DTO по правилу №6).

Требует ПРОДУКТОВОГО решения (задокументировано тестами, не чинилось):
- **2FA не действует на вход через мини-апп** (/auth/telegram/webapp выдаёт
  токены без TOTP-гейта; гейт вслепую заблокирует 2FA-юзеров в Telegram).
- TOTP-replay основного кода (см. ниже).
- Дубль-вызов saveYsqProgress при двойном тапе (парный файл, безвредно).
