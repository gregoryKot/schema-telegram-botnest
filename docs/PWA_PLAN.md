...
> **Пункт 1 пересмотрен 30.07.2026 — см. [MULTI_HOST_PLAN.md](MULTI_HOST_PLAN.md).**
> База для устанавливаемого приложения — миниапп, а не webapp: мост до хоста
> (`shared/src/host`) снял его завязку на Telegram initData, ради которой
> выбирали webapp. Остальные фазы этого плана в силе; service worker живёт со
> scope `/app/`, отчего риск «SW ломает миниапп и OAuth» отпадает сам.
