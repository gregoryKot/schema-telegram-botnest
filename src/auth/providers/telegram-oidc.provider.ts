...
export class TelegramOidcProvider implements AuthProviderHandler {
  ...
  async exchangeCodePkce(
    ...
    if (!tokenRes.ok) {
      ...
      this.logger.warn(
        `Telegram OIDC token exchange failed (${tokenRes.status}): ${body.slice(0, 200)}`,
      );
    ...
    if (!userRes.ok) {
      ...
      this.logger.warn(
        `Telegram OIDC userinfo failed (${userRes.status}): ${body.slice(0, 200)}`,
      );
