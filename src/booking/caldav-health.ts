...
// 401/403 детерминированы — открываем сразу; таймаут/сеть/http бывают разовыми.
...
export const KIND_TEXT: Record<CalDavFailureKind, string> = {
  auth: 'iCloud отвечает 401/403: не подходит пароль приложения ИЛИ запрос ушёл не в календарь (например, в корень …/calendars/)',
...
export class CalDavHealthTracker {
  ...
  noteFailure(
    ...
    return (
      ...
      (kind === 'auth'
        ? 'Похоже, дело в APPLE_APP_PASSWORD или в адресе APPLE_CALDAV_URL.\n'
        : '') +
      `Подробность (куда ушёл запрос): ${this.s.lastFailDetail}`
    );
  }
}
