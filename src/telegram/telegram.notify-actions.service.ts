...
import {
  ...
  CADENCE_LABELS,
} from '../notification/notification.cadence.service';
import {
  AddressForm,
...
function formatDay(d: Date, tz: string): string {
  ...
  const day = Number(
    parts.find((p) => p.type === 'day')?.value ?? d.getUTCDate(),
  );
  const month =
    Number(
      parts.find((p) => p.type === 'month')?.value ?? d.getUTCMonth() + 1,
    ) - 1;
  return `${day} ${MONTHS[month]}`;
}

function pauseConfirmText(
  days: number,
  dateStr: string,
  form: AddressForm,
): string {
  ...
  onModuleInit() {
    ...
    this.bot.action(/^addr:(ty|vy)$/, async (ctx) => {
      try {
        ...
        await ctx.editMessageText(
          `${ack}\n\n${t(form, WELCOME_TEXT, WELCOME_TEXT_VY)}`,
    ...
    this.bot.action(/^notify:pause:(7|30)$/, async (ctx) => {
      try {
        ...
        await ctx
          .editMessageText(pauseConfirmText(days, formatDay(until, tz), form))
          .catch(() => null);
      } catch (err) {
        this.logger.error('notify:pause:days failed', err);
        await ctx
          .answerCbQuery('Не получилось поставить паузу.')
          .catch(() => null);
      }
    });
