...
// пустых строк), полные данные (все три механики), частичные данные (только
// баннер / только лендинг / только запуски на компьютере — остальные строки
// не печатаются).
import {
...
describe('formatSiteInstallMetrics', () => {
  ...
  it('полные данные: баннер кабинета, лендинг и запуски на компьютере', () => {
    ...
    expect(text).toContain('Запускали на компьютере: 14 раз (6 человек)');
  });

  it('только лендинг — строки баннера и запусков на компьютере не печатаются', () => {
    ...
    expect(text).not.toContain('Запускали на компьютере');
  });

  it('только баннер — строки лендинга и запусков на компьютере не печатаются', () => {
    const text = formatSiteInstallMetrics({
  ...
  it('только запуски на компьютере — строки баннера и лендинга не печатаются', () => {
    const text = formatSiteInstallMetrics({
      banner: { shown30: 0, add30: 0, added30: 0 },
      landing: { add30: 0, added30: 0 },
      desktopOpens30: { opens: 3, users: 2 },
    });
    expect(text).not.toContain('Баннер в кабинете');
    expect(text).not.toContain('Лендинг');
    expect(text).toContain('Запускали на компьютере: 3 раз (2 человек)');
  });
  ...
  it('в тексте нет служебных ключей', () => {
    ...
    expect(text).not.toMatch(
      /site_banner|site_landing|home_screen_offer|desktop_app_open/,
    );
  });
});
