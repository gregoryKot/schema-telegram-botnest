...
import { buildWeeklySummaryText, renderLowStreakInsight } from './notification.templates';
...
export class NotificationPlannerService {
  ...
  async planDay(user: PlannerUser, now = new Date()) {
    ...
    if (pauseState === 'expired') {
      if (!await this.notifications.hasPending(uid, 'welcome_back')) {
        await this.notifications.schedule(uid, 'welcome_back', nextSendAt(hour, tz, now));
      }
      return;
    }
    ...
    const lapsingType =
      daysSince === 3 && level < 2 ? 'lapsing_3'
      : daysSince === 7 && level < 2 ? 'dormant_7'
      : daysSince === 30 ? 'reengagement_30'
      : daysSince > 30 && (daysSince - 30) % 45 === 0 ? 'nudge'
      : null;
    if (lapsingType) {
      if (!await this.notifications.hasPending(uid, lapsingType)) {
        ...
        await this.notifications.schedule(uid, lapsingType, nextSendAt(hour, tz, now), payload);
      }
      return;
    }
    ...
    if (isSunday && daysSince >= 0 && daysSince < 7 && !await this.notifications.hasPending(uid, 'weekly')) {
      const stats = await this.analytics.getWeeklyStats(uid);
      const bestDay = await this.analytics.getBestDayOfWeek(uid);
      const text = buildWeeklySummaryText(stats, this.botService.getNeeds(), bestDay, Number(uid % 3n), normalizeAddressForm(user.addressForm));
      await this.notifications.cancel(uid, 'reminder');
      await this.notifications.schedule(uid, 'weekly', nextSendAt(hour, tz, now), { text });
      return;
    }
    ...
    if (missed.length > 0 && !await this.notifications.hasPending(uid, 'practice_missed')) {
      const text = missed.map((p) => p.practiceText).join(', ');
      await this.notifications.schedule(uid, 'practice_missed', nextSendAt(hour, tz, now), { practiceText: text });
      return;
    }
  ...
  async scheduleReminder(userId: bigint, notifyLocalHour: number, tz: string, now = new Date()) {
    ...
    await this.notifications.schedule(userId, 'reminder', nextSendAt(notifyLocalHour, tz, now), payload);
  }

  private async scheduleLowStreakInsight(userId: bigint, notifyLocalHour: number, tz: string, now: Date, addressForm?: string | null) {
    ...
    if (await this.notifications.hasPending(userId, 'low_streak_insight')) return;

    const showBooking = lowNeeds10.length > 0;
    const need = this.botService.getNeeds().find((n) => n.id === lowNeeds[0])!;
    const { text } = renderLowStreakInsight(need.emoji, need.chartLabel, showBooking ? 10 : 3, normalizeAddressForm(addressForm));
    await this.notifications.schedule(
      userId, 'low_streak_insight', nextSendAt(notifyLocalHour, tz, now), { text, showBooking },
    );
  }
}
