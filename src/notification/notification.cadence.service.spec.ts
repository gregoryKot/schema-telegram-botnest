import { NotificationCadenceService, CadenceUser, effectiveLevel } from './notification.cadence.service';
...
describe('NotificationCadenceService', () => {
  describe('evaluate — детекция игнора', () => {
    it('инкрементирует счётчик когда напоминание отправлено вчера и дневник не заполнен', async () => {
      ...
      notifications.lastSentAt.mockResolvedValue(new Date('2026-07-01T18:00:00Z')); // 21:00 МСК вчера
      analytics.getDaysSinceLastFill.mockResolvedValue(5);
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
    ...
    it('НЕ считает игнором когда дневник заполнен вчера', async () => {
      const { prisma, notifications, analytics } = makeDeps();
      notifications.lastSentAt.mockResolvedValue(new Date('2026-07-01T18:00:00Z'));
    ...
    it('НЕ считает игнором когда вчера напоминания не было', async () => {
      const { prisma, notifications, analytics } = makeDeps();
      notifications.lastSentAt.mockResolvedValue(new Date('2026-06-28T18:00:00Z'));
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
      await svc.evaluate(makeUser(), NOW);
      expect(updatedData(prisma).notifyIgnoredCount).toBe(0);
    });
  ...
  describe('evaluate — сдвиги уровня', () => {
    ...
    it('уровень 3 — пол, ниже не падает', async () => {
      ...
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
      await svc.evaluate(makeUser({ notifyAdaptiveLevel: 3, notifyIgnoredCount: 2 }), NOW);
    ...
    it('НЕ поднимается ниже потолка notifyFrequency', async () => {
      ...
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
      await svc.evaluate(makeUser({ notifyFrequency: 2, notifyAdaptiveLevel: 2 }), NOW);
  ...
  describe('evaluate — день напоминания', () => {
    ...
    it('nextRemindDate в будущем → не сегодня', async () => {
      const { prisma, notifications, analytics } = makeDeps();
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
      const { remindToday } = await svc.evaluate(makeUser({ notifyNextRemindDate: '2026-07-05' }), NOW);
    ...
    it('интервал считается по эффективному уровню (недельный)', async () => {
      const { prisma, notifications, analytics } = makeDeps();
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
      const { remindToday } = await svc.evaluate(makeUser({ notifyFrequency: 3 }), NOW);
      expect(remindToday).toBe(true);
    ...
    it('идемпотентна: повторный прогон в тот же день ничего не меняет', async () => {
      const { prisma, notifications, analytics } = makeDeps();
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
      const { remindToday } = await svc.evaluate(makeUser({ notifyLastEvalDate: TODAY_MSK, notifyIgnoredCount: 2 }), NOW);
      expect(remindToday).toBe(false);
  ...
  describe('пауза', () => {
    it('активная пауза → paused', async () => {
      const { prisma, notifications, analytics } = makeDeps();
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
      const res = await svc.expirePauseIfDue(makeUser({ notifyPausedUntil: new Date('2026-07-10T00:00:00Z') }), NOW);
    ...
    it('истёкшая пауза → expired, состояние сброшено', async () => {
      const { prisma, notifications, analytics } = makeDeps();
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
      const res = await svc.expirePauseIfDue(makeUser({ notifyPausedUntil: new Date('2026-07-01T00:00:00Z'), notifyIgnoredCount: 2 }), NOW);
  ...
  describe('событийные апдейты', () => {
    it('registerFill сбрасывает счётчик и сдвигает следующий контакт', async () => {
      const { prisma, notifications, analytics } = makeDeps();
      prisma.user.findUnique.mockResolvedValue(makeUser({ notifyFrequency: 1 }));
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
    ...
    it('skipToday ставит только skipAck и отменяет pre_reminder', async () => {
      ...
      expect(notifications.cancel).toHaveBeenCalledWith(BigInt(42), 'pre_reminder');
    });
    ...
    it.each([
      ...
      const { prisma, notifications, analytics } = makeDeps();
      prisma.user.findUnique.mockResolvedValue(makeUser({ notifyFrequency: from, notifyAdaptiveLevel: from }));
      const svc = new NotificationCadenceService(prisma, notifications, analytics);
  ...
  describe('effectiveLevel', () => {
    it('берёт максимум из frequency и adaptive', () => {
      expect(effectiveLevel({ notifyFrequency: 1, notifyAdaptiveLevel: 2 })).toBe(2);
      expect(effectiveLevel({ notifyFrequency: 3, notifyAdaptiveLevel: 0 })).toBe(3);
      expect(effectiveLevel({ notifyFrequency: 0, notifyAdaptiveLevel: 0 })).toBe(0);
    });
  });
});
