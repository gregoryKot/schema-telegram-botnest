...
// ════════════════════════════════════════════════════════════════════════════
//  LEVEL 1 — "ОБЫЧНЫЙ ДЕНЬ" (life before therapy)
//  Core: fight / freeze / flee = the three nervous-system stress responses.
//  You have all three — and slowly discover none of them truly saves you.
//  That lived helplessness IS the message. No teaching text — only the cat's voice.
// ════════════════════════════════════════════════════════════════════════════
...
export class Level1Scene extends Phaser.Scene {
  ...
  private checkpoints = [100, 950, 1620, 2280, 3280, 4000];
  private readonly pits = [
  ...
  create() {
    Object.assign(this, {
      anx: [], critic: null, trail: [], hearts: 3, invuln: 0, checkpointX: 100,
  ...
  private buildBackground() {
    ...
    g.fillGradientStyle(0x161228, 0x161228, 0x3a2c4e, 0x3a2c4e, 1, 1, 1, 1);
    g.fillRect(0, 0, W, H);
    // distant glow lighting the play zone
    const glow = this.add.graphics().setScrollFactor(0).setDepth(-8);
    glow.fillStyle(0x7a4a8a, 0.30); glow.fillEllipse(W / 2, GROUND_Y - 10, W * 1.6, 320);
    glow.fillStyle(0x8a5a9a, 0.22); glow.fillEllipse(W / 2, GROUND_Y - 10, W * 1.0, 220);
    ...
    for (let layer = 0; layer < 2; layer++) {
      ...
      for (let i = 0; i < 14; i++) {
        const m = this.add.rectangle(Phaser.Math.Between(0, ARENA_W), Phaser.Math.Between(40, GROUND_Y - 30),
          2 + layer, 2 + layer, 0x8a7aaa, 0.18 + layer * 0.12);
        cont.add(m);
    ...
    fog.fillGradientStyle(0x1a1226, 0x1a1226, 0x1a1226, 0x1a1226, 0, 0, 0.55, 0.55);
  ...
  private buildGround() {
    ...
    for (let i = 0; i < n; i++) {
      const tx = i * tw;
      if (this.pits.some(p => tx < p.e && tx + tw > p.s)) continue;
      this.add.image(tx + tw / 2, GROUND_Y + th / 2, 'ground').setDepth(2).setTint(0xa49abf);
    }
    const edges = [0, ...this.pits.flatMap(p => [p.s, p.e]), ARENA_W];
  ...
  private buildPlatforms() {
    ...
    const G = GROUND_Y;
    const P = [
      { x: 360,  w: 120, y: G - 84  },  // A intro step
      { x: 800,  w: 110, y: G - 70  },  // cross pit1 (760-900)
      { x: 1120, w: 120, y: G - 92  },  // approach
      { x: 1340, w: 96,  y: G - 70  },  // cross pit2 low (1300-1520)
      { x: 1460, w: 90,  y: G - 150 },  // high — heart-in-jump
      { x: 1700, w: 130, y: G - 92  },
      { x: 1900, w: 110, y: G - 150 },  // heart spot
      { x: 2120, w: 100, y: G - 80  },  // cross pit3 (2050-2210)
      // ── climb (2400 → 2900) ──
      { x: 2400, w: 120, y: G - 70  },
      { x: 2560, w: 110, y: G - 120 },
      { x: 2700, w: 100, y: G - 168 },
      { x: 2850, w: 120, y: G - 214 },  // top — reward heart
      // ── wide pit4 (2950-3210) island ──
      { x: 3060, w: 96,  y: G - 132 },
      { x: 3270, w: 120, y: G - 80  },
      // ── gauntlet ──
      { x: 3540, w: 120, y: G - 134 },
      { x: 3780, w: 110, y: G - 92  },  // cross pit5 (3720-3870)
      { x: 4040, w: 150, y: G - 92  },
    ];
    for (const p of P) {
  ...
  private buildSpikes() {
    const defs = [{ x: 1140, w: 120 }, { x: 2300, w: 130 }, { x: 3360, w: 120 }];
    for (const d of defs) {
  ...
  private buildHearts() {
    const G = GROUND_Y;
    const spots = [
      { x: 420,  y: G - 116 }, // easy, on intro platform
      { x: 1460, y: G - 188 }, // high over pit2 — risky jump
      { x: 1900, y: G - 188 }, // mid platform
      { x: 2850, y: G - 252 }, // top of the climb — reward
      { x: 3110, y: G - 180 }, // over pit4 — risky
      { x: 4040, y: G - 132 }, // breather before the end
    ];
  ...
  private setupTriggers() {
    this.triggers = [
      { x: 280,  done: false, fn: () => { this.spawnAnx(this.player.x + 360, 1); this.say('...это ещё что?', 2200); audio.anx(); } },
      { x: 1780, done: false, fn: () => { this.spawnAnx(this.player.x + 360, 1); } },
      { x: 2520, done: false, fn: () => { this.spawnCritic(); } },
      { x: 3520, done: false, fn: () => { this.spawnAnx(this.player.x + 340, 1); this.say('сколько можно...', 2400); } },
      { x: 4320, done: false, fn: () => { this.beginOverwhelm(); } },
    ];
  }
  ...
  private fallRespawn() {
    const cp = [...this.checkpoints].reverse().find(c => c <= this.player.x) ?? 100;
    const b = this.player.body as Phaser.Physics.Arcade.Body;
  ...
  private updateMoves(dt: number) {
    ...
    if (this.dashing) {
      ...
      else if (right) { b.setVelocityX(RUN_SPEED); this.player.setFlipX(false); }
      else if (left)  { b.setVelocityX(-RUN_SPEED); this.player.setFlipX(true); }
  ...
  private beginOverwhelm() {
    ...
    for (let i = 0; i < 3; i++) this.spawnAnx(this.player.x + (i - 1) * 200 + Phaser.Math.Between(-40, 40), 1);
    this.say('сколько можно... я так больше не могу.', 3200);
  ...
  private showRealization() {
    ...
    // What just happened — the cat ran, looked for help, none came;
    // only the inner critic kept catching up, and hurt the most.
    line('Ты бежал. Искал, куда спрятаться.',                 116, '#d8c8ec', 17, 700);
    line('Звал на помощь. Помощь не пришла.',                 150, '#b9a6d6', 15, 2000);
    line('Что бы ты ни делал —\nдрался, замирал, убегал —',   226, '#d8c8ec', 16, 4000);
    line('внутренний критик всегда догонял.',                 278, '#ff8aa6', 16, 5400);
    line('Твоя же тень. И била больнее всех.',                312, '#e8b8c8', 15, 6700);
    line('Один на один с этим — так не живут.',               382, '#e8d0dc', 16, 8800);
    line('Но справляться в одиночку и не нужно.',             420, '#a8e8d0', 15, 10600);
    line('Этому учит терапия. Она — дальше.',                 462, '#88ffcc', 14, 12200);
    this.time.delayedCall(10600, () => audio.toll());

    // not a soft-lock: let the player restart after it settles
    this.time.delayedCall(14000, () => {
      const hint = this.add.text(W / 2, H - 28, 'нажми любую клавишу', { fontFamily: 'Courier New', fontSize: '11px', color: '#5a4f7a' })
        .setOrigin(0.5).setScrollFactor(0).setDepth(151).setAlpha(0);
