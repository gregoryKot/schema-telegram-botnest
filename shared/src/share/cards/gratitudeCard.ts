// Карточка записи благодарности. ЕДИНСТВЕННАЯ карточка со свободным текстом
// пользователя: шэрится только по явному действию на конкретной записи, и
// пользователь видит превью карточки перед отправкой (ShareCardSheet).
import {
  CARD_W,
  CARD_PAD,
  FOOTER_H,
