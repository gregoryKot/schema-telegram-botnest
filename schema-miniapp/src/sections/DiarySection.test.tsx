...
describe('DiarySection — сохранение записи благодарности (read-after-write)', () => {
  ...
  it('ошибка api.createGratitudeDiary НЕ добавляет запись в раздел сохранённых (не выдаёт черновик за сохранённую запись)', async () => {
    ...
    // Текст остаётся видимым ТОЛЬКО как черновик (карточка с меткой
    // «Черновик» — пользователь его не теряет и может продолжить), но не
    // как реальная сохранённая запись — счётчик дневника не увеличился и
    // объясняющий пустое состояние блок для "0 сохранённых" всё ещё на месте.
