...
export function NotifyInfoOverlay({ onClose }: { onClose: () => void }) {
  return (
    <InfoSheetShell title="Зачем уведомления" onClose={onClose}>
      ...
      >
        <b className="u-fg">Итоги дня</b> — приходят в это же время, если
        дневник заполнен.
      </p>
    </InfoSheetShell>
  );
  ...
  return (
    <InfoSheetShell title="Зачем привязывать друга" onClose={onClose}>
      ...
      >
        ...
        <b className="u-fg">индексы дня</b> друг друга — просто число от 0 до
        10. Никаких деталей, дневников или оценок.
      </p>
      <p
        style={{
          fontSize: 14,
...
export function TherapistInfoOverlay({ onClose }: { onClose: () => void }) {
  ...
  return (
    <InfoSheetShell title="Зачем подключать терапевта" onClose={onClose}>
      ...
      >
        {tr('Если ты работаешь', 'Если вы работаете')} с терапевтом в подходе
        схема-терапия — приложение может стать частью этой работы.
      </p>
      <p
        style={{
          fontSize: 14,
      ...
      >
        ...
        <b className="u-fg">трекер потребностей и задания</b>.{' '}
        {tr(
