...
export function PracticesOnboarding({ needs, onDone }: Props) {
  ...
  async function handleSaveAndNext() {
    ...
    try {
      const results = await Promise.allSettled(
        toSave.map((text) => api.addPractice(needId, text)),
      );
  ...
  const currentNeed = step !== 'intro' ? needs[step] : null;
  const color = currentNeed
    ? (COLORS[currentNeed.id] ?? '#888')
    : 'var(--accent)';
  const NEED_DATA = useNeedData();
  const emoji = currentNeed ? (NEED_DATA[currentNeed.id]?.emoji ?? '') : '';
  const total = needs.length;
  const progress = step === 'intro' ? 0 : (step + 1) / total;
        ...
        <div>
          <div style={{ textAlign: 'center', marginBottom: 20, paddingTop: 4 }}>
            ...
            >
              ...
              <span style={{ color: 'rgba(var(--fg-rgb),0.8)' }}>
                Добавь заранее
              </span>{' '}
              — и они будут под рукой в нужный момент.
            </div>
          </div>
          ...
          >
            ...
            >
              В схема-терапии это называют{' '}
              <span style={{ color: 'var(--accent)', fontWeight: 500 }}>
                копинг-карточками
              </span>{' '}
              — маленькими напоминаниями себе о том, что реально работает. Не
              воля, а конкретный шаг.
            </div>
          </div>
          ...
          <div>
            ...
            >
              {tr(
                ...
                'Выберите готовые или добавьте своё:',
              )}
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontSize: 14,
                        color: on ? '#fff' : 'rgba(var(--fg-rgb),0.7)',
                        lineHeight: 1.4,
                      }}
                    >
                      {text}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              >
                {step === total - 1 ? 'Готово' : 'Пропустить →'}
              </button>
              <button
                ...
                style={{
                  ...
                  fontWeight: 600,
                  cursor:
                    selected.size > 0 || input.trim() ? 'pointer' : 'default',
                  transition: 'background 0.15s',
                }}
              >
                {saving
                  ? '...'
                  : `Сохранить${selected.size + (input.trim() ? 1 : 0) > 0 ? ` (${selected.size + (input.trim() ? 1 : 0)})` : ''} →`}
              </button>
            </div>
          </div>
        )
