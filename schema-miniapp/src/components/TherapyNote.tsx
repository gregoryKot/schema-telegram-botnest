...
export function TherapyNote({ compact }: { compact?: boolean }) {
  ...
  const linkLabel = contact.isTherapist
    ? tr('Ты специалист — клиенты могут обратиться', 'Вы специалист — клиенты могут обратиться')
    : contact.name === 'автору'
    ...
    <div style={{
      ...
      <div>
        <div style={{ fontSize: 12, color: 'var(--text-sub)', lineHeight: 1.55 }}>
          {contact.isTherapist
            ? tr('Ты работаешь как терапевт. Клиенты обращаются к тебе — ты уже рядом.', 'Вы работаете как терапевт. Клиенты обращаются к вам — вы уже рядом.')
            : tr('Это инструмент самоисследования — не клиническая диагностика и не замена работе с психологом. Если чувствуешь, что нужно разобраться глубже — терапия это место где безопасно.', 'Это инструмент самоисследования — не клиническая диагностика и не замена работе с психологом. Если чувствуете, что нужно разобраться глубже — терапия это место где безопасно.')
          }
        </div>
        <a
          href={contact.url}
