...
describe('TodayCustomizeSheet — строки band', () => {
  ...
  // Свод онбординга 2026-08-31: шаг визарда today_screen снят — единственный
  // уникальный факт этого шага («задержишь палец на любом блоке — настройка
  // откроется сразу на нём») жил ТОЛЬКО там, если бы его убрали без переноса,
  // жест остался бы без аффорданса. Строка-подсказка уже жила в этом листе
  // (см. секцию «Показывать на главном») — проверяем явно, чтобы факт не
  // потерялся молча при следующей правке.
  it('подсказка про долгое нажатие видна', () => {
    render(<TodayCustomizeSheet {...baseProps()} />);
    expect(
      screen.getByText(/долгое нажатие на любой блок главного экрана/),
    ).toBeTruthy();
  });
