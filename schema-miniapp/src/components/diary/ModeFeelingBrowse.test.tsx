...
// Переиспользует данные ворот shared/mode/modeFeelGates — синхронность с
// MODE_GROUPS уже покрыта modeTest.test.ts, здесь проверяем только
// взаимодействие.
...
describe('ModeFeelingBrowse', () => {
  ...
  it('раскрытие семьи показывает вопрос-уточнение и hint над списком режимов', () => {
    render(<ModeFeelingBrowse onChange={vi.fn()} />);
    fireEvent.click(screen.getByText(/Страшно, тревожно/));
    // question — новая строка над hint (редизайн ворот-по-чувствам).
    expect(screen.getByText('Про что этот страх?')).toBeTruthy();
    // hint — тёплое описание из данных (уже было в ModeTestLeaf, потерялось при регрессе).
    expect(
  ...
  it('повторный тап по открытой семье сворачивает список', () => {
    render(<ModeFeelingBrowse onChange={vi.fn()} />);
    const chip = screen.getByText(/Страшно, тревожно/);
  ...
  it('клик по чипу «Пусто или не пойму, что чувствую» показывает лист «Пусто и ровно, как в вате» с desc, клик по нему выбирает detached_protector', () => {
    const onChange = vi.fn();
    render(<ModeFeelingBrowse onChange={onChange} />);
