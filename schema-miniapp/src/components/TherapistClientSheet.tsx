...
export function TherapistClientSheet({ view, onViewChange, onClose, backHandlerRef }: Props) {
  ...
  useEffect(() => {
    ...
    backHandlerRef.current = () => {
      ...
      if (detail.showConceptSheet)     { detail.setShowConceptSheet(false);     return; }
      if (detail.showTasksSheet)       { detail.setShowTasksSheet(false);       return; }
      if (detail.showNotesSheet)       { detail.setShowNotesSheet(false);       return; }
      if (detail.showClientNotesSheet) { detail.setShowClientNotesSheet(false); return; }
      switchView('list');
    };
  }, [view, detail.showAssign, detail.showConceptSheet, detail.showTasksSheet, detail.showNotesSheet, detail.showClientNotesSheet, backHandlerRef]);

  // ─── Destructure for JSX convenience ─────────────────────────────────────────
  const {
    selectedClient, setSelectedClient,
    showTasksSheet, setShowTasksSheet,
    showNotesSheet, setShowNotesSheet,
    showConceptSheet, setShowConceptSheet,
    showClientNotesSheet, setShowClientNotesSheet,
    clientSchemaNotesData, clientModeNotesData,
    clientTasks, setClientTasks,
    notes, noteError, setNoteError,
    concept, clientData,
    localConcept, setLocalConcept, conceptDirty, setConceptDirty, conceptSaving, conceptError,
    ...
    editingDays, setEditingDays,
    localMeetingDays, setLocalMeetingDays,
    sessionInfoSaving, sessionInfoError,
    renamingAlias, setRenamingAlias,
    aliasInput, setAliasInput,
    aliasSaving, aliasError, setAliasError,
    ysqRequested, setYsqRequested, ysqError,
    exportCopied,
    deleteLoading, deleteError,
    activeSchemaIds, activeModeIds, ysqSchemaIds, selfSchemaIds,
    openClient, deleteClient,
    addNote, removeNote,
    patchConcept, toggleSchemaId, toggleModeId, saveConcept,
  ...
  const {
    ...
    inviteLoading,
    inviteInputRef,
    openAddMode, createInvite, copyInvite, shareInvite, addByTelegramId, addVirtualClient,
  } = addClient;
