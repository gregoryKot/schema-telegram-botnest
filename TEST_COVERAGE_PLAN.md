...
8. ☐ Кризисная детекция: расширить корпус `crisisMarkers.test.ts` — слитное
   написание («нехочу жить»), прошедшее время («покончила» сейчас не
   матчится), эвфемизмы, латиница-микс; негативные кейсы как
   тесты-документация осознанных компромиссов. Самый важный тест продукта.
9. ☐ Бутстрап тестов miniapp: vitest + jsdom + `"test"`-скрипт + шаг в
   CI-джобу `miniapp`; setup-мок `window.Telegram.WebApp`; первые тесты —
   `useTelegramBackButton`, `useSheets`, смок `api.ts` (603 строки, разошёлся
   с webapp-версией). Альтернатива стратегически лучше: shared-пакет для
   парных файлов — тесты пишутся один раз.
10. ☐ webapp: `api.ts` (refresh/401), `addressForm.tsx` (`pickForm`,
    дефолт null→'ty'), `useHistorySheet` (задокументированный класс багов),
    `AuthContext.tsx`. Добавить `@vitest/coverage-v8` и test-секцию в
    vite.config (убрать пер-файловые прагмы jsdom).
