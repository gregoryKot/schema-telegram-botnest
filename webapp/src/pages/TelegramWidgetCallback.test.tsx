...
// Callback от oauth.telegram.org: данные приходят в hash-фрагменте (не видны
// серверу), декодируются на клиенте и уходят на /api/auth/telegram/widget.
// Три исхода (успех/TOTP/merge) + разбор ошибок — образец мокинга
// window.location: AuthCallback.test.tsx.
...
function renderWith(hash: string, auth: AuthState = authValue()) {
...
describe('TelegramWidgetCallback — нет данных / битые данные', () => {
  it('без tgAuthResult в хэше — уходит на /auth/error?reason=telegram_no_data', async () => {
...
describe('TelegramWidgetCallback — успешный вход', () => {
  ...
  it('сохранённый auth_return_to (из мини-аппа) используется вместо /today', async () => {
    ...
    Object.defineProperty(window, 'location', {
      configurable: true,
      value: { ...originalLocation, hash: `#tgAuthResult=${b64url({ id: 1, hash: 'x' })}` },
    });
