...
// хвосте). Решение владельца: неделя начинается с ВОСКРЕСЕНЬЯ — фиксируем
// под фейковым временем, а не полагаясь на реальную дату прогона теста.
describe('CalendarWeek — неделя с воскресенья', () => {
  beforeEach(() => {
  ...
  it('открыт в воскресенье — первый запрос с сегодняшнего воскресенья (2026-09-20), а не с прошлого понедельника', async () => {
    render(<CalendarWeek adminKey="k" />);
  ...
  it('открыт в среду — та же неделя с воскресенья 20-го (прошедшие дни остаются в сетке серыми)', async () => {
    vi.setSystemTime(new Date('2026-09-23T10:00:00.000Z')); // среда
    render(<CalendarWeek adminKey="k" />);
    await waitFor(() => expect(mockApi.adminCalendar).toHaveBeenCalled());
