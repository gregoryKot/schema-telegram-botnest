...
// Единственный источник для десктоп-навбара и мобильного меню
// (ProductMobileMenu.tsx) — раньше список ссылок жил только инлайном
// в ProductLandingPage, дублировать его во втором месте было бы разъездом.
...
export const STEPS = [
  ...
  { num: '02', color: AMBER,  emoji: '🔁', title: 'Они включаются незаметно', text: 'Во взрослой жизни схема срабатывает раньше, чем вы успеваете подумать: те же ссоры, та же тревога, та же самокритика, прокрастинация — реакция уже случилась, разбираться в причинах поздно.' },
...
export const FEATURES = [
  ...
  { emoji: '🧩', color: VIOLET,  title: 'Диагностика схем', text: 'Тест по схемам: 20 ранних дезадаптивных схем в пяти доменах — с понятным разбором, а не голыми цифрами.' },
...
export const FAQ: Array<{ q: string; a: ReactNode }> = [
  ...
  { q: 'Сколько это стоит?', a: 'Нисколько. Все функции бесплатны — без пробных периодов и платных уровней. Проект существует на добровольные донаты.' },
  {
    ...
    a: (
      <>
        Приложение делает Григорий Котляревский — психолог, работает в подходе
        схема-терапия, частная практика онлайн. «Всё по схеме» выросло из этой
        практики: те же инструменты самонаблюдения, которые помогают клиентам
        между сессиями, — бесплатно и для всех. Познакомиться с автором можно на{' '}
        <a
          href={AUTHOR_SITE}
          target="_blank"
