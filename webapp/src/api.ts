...
export interface HealthyAdultPhrase {
  id: number;
  text: string;
  enabled: boolean;
  sortOrder: number;
...
export interface SiteContent {
  ...
  marqueeTopicsA: MarqueeTopic[];
  marqueeTopicsB: MarqueeTopic[];
}
...
export const api = {
  ...
  saveRating: async (
    ...
    value: number,
    date?: string,
  ): Promise<{ ok: boolean; allDone: boolean; streak?: any }> => {
  ...
  getPractices: (needId: string) =>
    get<any[]>(`/api/practices?needId=${needId}`),
  ...
  createGratitudeDiary: (date: string, items: string[]) =>
    postJson<any>('/api/diary/gratitude', { date, items }),
  ...
  addClientManually: (clientTelegramId: number) =>
    postJson<any[]>('/api/therapy/clients/add', { clientTelegramId }),
  addVirtualClient: (name: string) =>
    postJson<any[]>('/api/therapy/clients/virtual', { name }),
  ...
  getTherapyTasksForClient: (clientId: number) =>
    get<any[]>(`/api/therapy/tasks/client/${clientId}`),
  getAllTherapyTasks: () =>
    get<{ clientId: number; clientName: string; tasks: UserTask[] }[]>(
      '/api/therapy/tasks/all',
    ),
  getTherapistNotes: (clientId: number) =>
    get<any[]>(`/api/therapy/notes/${clientId}`),
  createTherapistNote: (clientId: number, date: string, text: string) =>
    postJson<any>(`/api/therapy/notes/${clientId}`, { date, text }),
  ...
  saveConceptualization: (
    ...
    body: Partial<
      Omit<
        ...
        'id' | 'therapistId' | 'clientId' | 'history' | 'updatedAt'
      >
    >,
  ) =>
  ...
  updateSessionInfo: (clientId: number, body: any) =>
    post(`/api/therapy/session-info/${clientId}`, body),
  getTherapyClientData: (clientId: number) =>
    get<any>(`/api/therapy/client-data/${clientId}`),
  getTherapyClientHistory: (clientId: number) =>
    get<
      { date: string; index: number | null; ratings: Record<string, number> }[]
    >(`/api/therapy/client-history/${clientId}`),
  ...
  getClientSchemaNotes: (clientId: number) =>
    get<any[]>(`/api/therapy/client/${clientId}/schema-notes`),
  getClientModeNotes: (clientId: number) =>
    get<any[]>(`/api/therapy/client/${clientId}/mode-notes`),
  getClientDiary: (clientId: number) =>
    get<
      {
    ...
    >(`/api/therapy/client/${clientId}/diary`),
  submitBooking: (body: { name: string; contact: string; message?: string }) =>
    postJson<{ ok: true }>('/api/booking', body),
  // Slot-based booking
  ...
  cancelBooking: (token: string) =>
    postJson<{ ok: true }>(`/api/booking/cancel/${token}`, {}),
  donate: (body: {
    amount: number;
    source?: 'app' | 'game';
    email?: string;
  ...
  getSubscriptionOptions: () =>
    get<{
      enabled: boolean;
      options: { period: 'month' | 'year'; price: number }[];
  ...
  subscribe: (body: {
    ...
    postJson<{ id: number; cancelToken: string; paymentUrl: string | null }>(
      '/api/subscription',
      body,
    ),
  getSubscriptionByToken: (token: string) =>
    get<{
      status: string;
      period: string;
      amount: number;
      nextChargeAt: string | null;
    }>(`/api/subscription/by-token/${token}`),
  cancelSubscription: (token: string) =>
    postJson<{ ok: true }>(`/api/subscription/cancel/${token}`, {}),
  // Booking admin — key travels in the x-admin-key header (never in URL/logs)
  adminStatus: (key: string) =>
    adminReq<Record<string, any>>('GET', '/api/booking/admin/status', key),
  adminGetPrices: (key: string) =>
    adminReq<SessionOption[]>('GET', '/api/booking/admin/prices', key),
  adminSetPrice: (
    key: string,
    type: 'INTRO_15' | 'SESSION_50',
    amount: number,
  ) =>
    adminReq<{ ok: true }>('PATCH', '/api/booking/admin/price', key, {
      type,
      amount,
    }),
  adminGetSubPrices: (key: string) =>
    adminReq<{ period: 'month' | 'year'; price: number }[]>(
      'GET',
      '/api/booking/admin/sub-prices',
      key,
    ),
  adminSetSubPrice: (key: string, period: 'month' | 'year', amount: number) =>
    adminReq<{ ok: true }>('PATCH', '/api/booking/admin/sub-price', key, {
      period,
      amount,
    }),
  adminListRules: (key: string) =>
    adminReq<AvailabilityRule[]>('GET', '/api/booking/admin/rules', key),
  adminCreateRule: (key: string, rule: NewAvailabilityRule) =>
    adminReq<AvailabilityRule>('POST', '/api/booking/admin/rules', key, rule),
  adminToggleRule: (key: string, id: number, isActive: boolean) =>
    adminReq<AvailabilityRule>('PATCH', `/api/booking/admin/rules/${id}`, key, {
      isActive,
    }),
  adminDeleteRule: (key: string, id: number) =>
    adminReq<void>('DELETE', `/api/booking/admin/rules/${id}`, key),
  adminListBookings: (
    key: string,
    filter: 'upcoming' | 'past' | 'cancelled' | 'all' = 'upcoming',
  ) =>
    adminReq<AdminBooking[]>(
      'GET',
      `/api/booking/admin/list?filter=${filter}`,
      key,
    ),
  adminConfirm: (key: string, id: number) =>
    adminReq<{ ok: true }>('POST', `/api/booking/admin/confirm/${id}`, key),
  // Articles
  listArticles: () => get<ArticleSummary[]>('/api/articles'),
  getArticle: (slug: string) => get<Article>(`/api/articles/${slug}`),
  adminListArticles: (key: string) =>
    adminReq<Article[]>('GET', '/api/articles/admin/list', key),
  adminCreateArticle: (key: string, dto: ArticleDto) =>
    adminReq<Article>('POST', '/api/articles/admin', key, dto),
  adminUpdateArticle: (key: string, id: number, dto: Partial<ArticleDto>) =>
    adminReq<Article>('PATCH', `/api/articles/admin/${id}`, key, dto),
  adminDeleteArticle: (key: string, id: number) =>
    adminReq<void>('DELETE', `/api/articles/admin/${id}`, key),
  // Site content (hero photo, marquee topics)
  getSiteContent: () => get<SiteContent>('/api/site-content'),
  adminSetHeroPhoto: (key: string, dataUri: string) =>
    adminReq<{ ok: true }>('PATCH', '/api/site-content/admin/hero-photo', key, {
      dataUri,
    }),
  adminSetMarquee: (key: string, group: 'A' | 'B', topics: MarqueeTopic[]) =>
    adminReq<{ ok: true }>('PATCH', '/api/site-content/admin/marquee', key, {
      group,
      topics,
    }),
  // Healthy-adult channel phrases
  adminListPhrases: (key: string) =>
    adminReq<HealthyAdultPhrase[]>('GET', '/api/healthy-adult/admin/list', key),
  adminCreatePhrase: (key: string, text: string) =>
    adminReq<HealthyAdultPhrase>('POST', '/api/healthy-adult/admin', key, {
      text,
    }),
  adminUpdatePhrase: (
    key: string,
    ...
    adminReq<HealthyAdultPhrase>(
      'PATCH',
      `/api/healthy-adult/admin/${id}`,
      key,
      patch,
    ),
  adminDeletePhrase: (key: string, id: number) =>
    adminReq<void>('DELETE', `/api/healthy-adult/admin/${id}`, key),
  adminTestPhrasePost: (key: string, slot: 0 | 1) =>
    adminReq<{ ok: boolean; message: string }>(
      'POST',
      '/api/healthy-adult/admin/test-post',
      key,
      { slot },
    ),
  // Therapist custom modes
  listCustomModes: () =>
    get<TherapistCustomMode[]>('/api/therapy/custom-modes'),
  createCustomMode: (body: {
    name: string;
    emoji?: string;
    nodeType?: string;
  }) => postJson<TherapistCustomMode>('/api/therapy/custom-modes', body),
  deleteCustomMode: (id: number) => del(`/api/therapy/custom-modes/${id}`),
  // Mode Maps
  listModeMaps: (clientId: number) =>
    get<ModeMapMeta[]>(`/api/therapy/mode-maps/${clientId}`),
  getModeMap: (mapId: number) =>
    get<ModeMapFull>(`/api/therapy/mode-maps/map/${mapId}`),
  createModeMap: (
    clientId: number,
    title: string,
    kind: ModeMapKind = 'problem',
  ) =>
    postJson<ModeMapFull>(`/api/therapy/mode-maps/${clientId}`, {
      title,
      kind,
    }),
  updateModeMap: (
    mapId: number,
    body: Partial<Pick<ModeMapFull, 'title' | 'nodes' | 'edges'>>,
  ) => patchJson<ModeMapFull>(`/api/therapy/mode-maps/map/${mapId}`, body),
  ...
  getMyModeMap: (mapId: number) =>
    get<ModeMapFull>(`/api/therapy/my-mode-maps/${mapId}`),
};
