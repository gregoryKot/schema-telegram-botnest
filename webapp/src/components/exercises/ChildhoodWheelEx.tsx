...
const NEEDS = [
  {
    ...
    color: 'var(--c-rose)',
    question:
      'Был ли рядом взрослый, который замечал, когда было плохо, и откликался?',
    low: 'Взрослый отсутствовал или был непредсказуем. В трудный момент некому было обратиться.',
    high: 'Был стабильный взрослый, которому можно было доверять. Оставалось чувство нужности.',
  },
  {
    id: 'autonomy',
    label: 'Автономия',
    color: 'var(--c-clay)',
    question:
      'Было ли доверие – иметь своё мнение, делать выборы по возрасту, ошибаться?',
    low: 'Контролировали, решали за ребёнка, критиковали. Ошибка была катастрофой.',
  ...
  },
  {
    ...
    color: 'var(--c-amber)',
    question:
      'Были ли правила в семье последовательными и справедливыми, не случайными?',
    ...
    >
      <div className="cw-track-line">
        ...
        {[0, 25, 50, 75, 100].map((p) => (
          <div key={p} className="cw-track-pip" style={{ left: p + '%' }} />
        ))}
      </div>
...
function WheelSvg({ ratings }: { ratings: Record<string, number> }) {
  const size = 240,
    ...
    cy = 120,
    maxR = 80;
  const angles = NEEDS.map(
    (_, i) => (i * 2 * Math.PI) / NEEDS.length - Math.PI / 2,
  );
  const pts = NEEDS.map((n, i) => {
      ...
      {[0.25, 0.5, 0.75, 1].map((p) => (
        <polygon
          ...
          points={angles
            .map(
              (a) =>
                `${(cx + maxR * p * Math.cos(a)).toFixed(1)},${(cy + maxR * p * Math.sin(a)).toFixed(1)}`,
            )
            .join(' ')}
          fill="none"
          stroke="var(--line)"
      ...
      {NEEDS.map((n, i) => {
        ...
        return (
          <g key={n.id}>
            ...
            />
            <text
              x={(cx + (maxR + 22) * Math.cos(angles[i])).toFixed(1)}
              y={(cy + (maxR + 22) * Math.sin(angles[i])).toFixed(1)}
              textAnchor="middle"
...
export function ChildhoodWheelEx({
  onBack,
  onSaved,
}: {
  onBack: () => void;
  ...
  const avg =
    Math.round(
      (Object.values(ratings).reduce((a, b) => a + b, 0) / NEEDS.length) * 10,
    ) / 10;
  ...
  if (done) {
    return (
      <ExScreen
        onBack={goBack}
        eyebrow="Колесо детства · сохранено"
        eyebrowColor="var(--accent-indigo)"
        title={
          <>
            Колесо
            <br />
            <span className="it">детства</span>
          </>
        }
          ...
          <div className="cw-sliders">
            {NEEDS.map((n, i) => (
              <div
                key={n.id}
                className="cw-row"
                style={{ '--c-color': n.color } as React.CSSProperties}
              >
                <span className="cw-row-num">
                  {String(i + 1).padStart(2, '0')}
                </span>
                <div className="cw-row-body">
                  <div className="cw-row-label">{n.label}</div>
                  <div className="cw-row-q" style={{ marginBottom: 0 }}>
                    {ratings[n.id] <= 4
                      ? n.low
                      : ratings[n.id] >= 8
                        ? n.high
                        : 'Где-то между. Бывало по-разному.'}
                  </div>
                </div>
                <div className="cw-row-value">
                  {ratings[n.id]}
      ...
      aside={
        <>
          ...
          >
            ...
            <p className="body">
              Психика прячет систематические паттерны и подсвечивает редкие
              хорошие моменты. Оценивай не «как могло быть», а «как было обычно,
              в большинстве лет».
            </p>
          </div>
            ...
            >
              <div
                style={{
                  ...
                  lineHeight: 1.55,
                  color:
                    ratings[n.id] <= 4 ? 'var(--c-rose)' : 'var(--text-faint)',
              ...
              <div
                style={{
                  ...
                  lineHeight: 1.55,
                  color:
                    ratings[n.id] >= 8 ? 'var(--c-moss)' : 'var(--text-faint)',
