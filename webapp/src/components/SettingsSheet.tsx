...
export function SettingsSheet({
  ...
  const [therapistReq, setTherapistReq] = useState<
    { status: string; rejectReason: string | null } | null | undefined
  >(undefined);
  ...
  async function submitTherapistRequest() {
    ...
    try {
      ...
      setReqError(String(e).replace('Error: ', ''));
    } finally {
      setReqBusy(false);
    }
  }
    ...
    <>
        ...
        <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            <div
              ...
              style={{
                padding: subView !== 'main' ? '32px 48px 80px' : '0 48px 80px',
              }}
            >
              {/* ── TIME VIEW ── */}
              {subView === 'time' && (
              ...
              {subView === 'quiet' && (
                <>
                  ...
                  >
                    В тихие часы бот не пишет вообще — всё, что накопится,
                    придёт утром.
                  </div>
                  <div
                    style={{ display: 'flex', flexDirection: 'column', gap: 2 }}
              ...
              {subView === 'main' && (
                <>
                  ...
                  <SHead
                    id="s-notifications"
                    label="Уведомления"
                    hint="Приходят через Telegram — @SchemaLabBot"
                  />
                  ...
                  {(settings.notifyEnabled ||
                    ...
                    <>
                      <SRow
                        title="Время"
                        right={<ChevronVal text={`${pad(localHour)}:00`} />}
                      ...
                      <SRow
                        ...
                        onClick={() => setSubView('quiet')}
                      />
                      <SRow
                        title="Часовой пояс"
                        right={<ChevronVal text={tzLabel} small />}
                        onClick={() => setSubView('tz')}
                      />
                      {hourInQuiet(
                        localHour,
                        settings.notifyQuietStart,
                        ...
                        >
                          Время уведомления попадает в тихие часы — сообщение
                          придёт после их окончания
                        </div>
                      )}
                    </>
                  )}
                  ...
                  {userRole !== 'THERAPIST' && (
                    <>
                      <SHead
                        id="s-therapist"
                        label="Мой терапевт"
                        hint="Терапевт видит трекер и задания. Остальное — на твоё усмотрение."
                      />
                      {therapyRelation === undefined ? (
                        ...
                        >
                          ...
                          >
                            {therapyRelation.partnerName ?? 'Терапевт'}{' '}
                            подключён
                          </div>
                          <SRow
                            title="Карточки схем и режимов"
                            sub="Личные карточки и заметки"
                            right={
                              <SmallToggle
                                on={!!settings.therapistShareCards}
                                onClick={() =>
                                  patch({
                                    therapistShareCards:
                          ...
                          />
                          <SRow
                            title="Профиль и схемы"
                            sub="Активные схемы и результаты теста"
                            right={
                              <SmallToggle
                                on={!!settings.therapistShareProfile}
                                onClick={() =>
                                  patch({
                        ...
                        >
                          ...
                          >
                            Если терапевт выслал ссылку-приглашение — введи код
                            ниже.
                          </p>
                          <p
                            style={{
                          ...
                          <div style={{ display: 'flex', gap: 8 }}>
                            <input
                              ...
                              style={{
                                ...
                                border: `1px solid ${therapyJoinError ? 'var(--accent-red)' : 'rgba(var(--fg-rgb),0.1)'}`,
                                borderRadius: 7,
                                padding: '8px 12px',
                                color: 'var(--text)',
                                fontSize: 14,
                    ...
                    <>
                      ...
                      >
                        {therapistReq?.status === 'pending' ? (
                          ...
                          >
                            ⏳ Заявка на рассмотрении. Когда администратор
                            одобрит — придёт уведомление в Telegram.
                          </div>
                          ...
                          >
                            ✅ Заявка одобрена. Перезайди в приложение чтобы
                            появился кабинет терапевта.
                          </div>
                        ) : !showReqForm ? (
                          <div>
                            ...
                            >
                              Если ты практикующий специалист — подай заявку.
                              Администратор проверит и откроет доступ к
                              кабинету.
                            </p>
                            <button
                          ...
                          >
                            ...
                            <textarea
                              value={reqQual}
                              onChange={(e) => setReqQual(e.target.value)}
                              rows={3}
                            ...
                            <input
                              value={reqContacts}
                              onChange={(e) => setReqContacts(e.target.value)}
                              placeholder="Контакты: сайт, @telegram, b17 и т.д."
                              style={inputStyle}
                            />
                            <textarea
                              value={reqMsg}
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                              ...
                              <button
                                disabled={reqBusy}
                                onClick={submitTherapistRequest}
                                style={{
                                  flex: 2,
                                  padding: '10px 0',
                                  borderRadius: 7,
                  ...
                  {userRole === 'THERAPIST' && (
                    <>
                      ...
                      <SRow
                        ...
                        sub="Клиенты, задания, приглашения"
                  ...
                  <SHead
                    id="s-partner"
                    label="Партнёр"
                    hint="Видите индексы дня друг друга — просто число, без деталей"
                  />
                  <div
                    style={{
                      padding: '16px 0',
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          <input
                            ...
                            style={{
                              ...
                              borderRadius: 7,
                              background: 'rgba(var(--fg-rgb),0.05)',
                              border: `1px solid ${joinError ? 'var(--accent-red)' : 'rgba(var(--fg-rgb),0.1)'}`,
                              color: 'var(--text)',
                              fontSize: 15,
                              fontFamily: 'monospace',
                              outline: 'none',
                  ...
                  />
                  <SRow
                    title="Сводка для терапевта"
                    sub="Данные за 30 дней"
                    onClick={async () => {
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              color: exportCopied ? 'var(--accent-green)' : 'var(--text-sub)',
              fontSize: 14,
              fontWeight: 600,
              cursor: 'pointer',
              fontFamily: 'inherit',
      ...
      {showPrivacy && (
        <InfoModal onClose={() => setShowPrivacy(false)}>
          ...
          {[
            {
              title: 'Что хранится',
              text: 'Дневник, оценки, заметки, практики, результаты тестов — всё привязано к аккаунту и доступно с любого устройства.',
            },
            {
              title: 'Передача третьим лицам',
              text: 'Данные не продаются и не передаются. Никогда.',
            },
          ].map((b) => (
            <div
              key={b.title}
      ...
      {showDeleteSheet && (
        ...
        >
          ...
          >
            Дневники, оценки, практики, тесты, заметки, задания — всё удалится с
            сервера. Необратимо.
          </div>
          {!deleteConfirm ? (
  ...
  return (
    ...
    >
      ...
      {right ??
        (onClick && (
          <span
            style={{ color: 'var(--text-faint)', fontSize: 16, flexShrink: 0 }}
          >
            ›
          </span>
        ))}
