...
export function TrackerOverlay({
    ...
    return (
      <div style={{ position: 'fixed', inset: 0, zIndex: 80, background: 'var(--bg)',
        ...
        <div style={{ textAlign: 'center', maxWidth: 320 }}>
          ...
          <h2 style={{ fontFamily: 'var(--serif)', fontSize: 44, fontWeight: 400,
            color: 'var(--text)', lineHeight: 1.0, marginBottom: 20, letterSpacing: '-0.02em' }}>
            Заполнено
          </h2>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'center', gap: 4, marginBottom: 6 }}>
            <span style={{ fontFamily: 'var(--serif)', fontSize: 72, color: 'var(--text)',
              lineHeight: 1, letterSpacing: '-0.03em', fontWeight: 400 }}>
              {avgIdx.toFixed(1)}
            </span>
            <span style={{ fontSize: 20, color: 'var(--text-sub)', paddingBottom: 6 }}>/10</span>
          </div>
          <div style={{ fontSize: 11, color: 'var(--text-faint)', letterSpacing: '0.10em',
            textTransform: 'uppercase', fontWeight: 600, marginBottom: 36 }}>индекс дня</div>
          <button onClick={isBackfill ? (onDone ?? goBack) : goBack}
            style={{ padding: '13px 40px', borderRadius: 10, border: 'none',
              background: 'var(--text)', color: 'var(--bg)', fontSize: 15, fontWeight: 600,
              cursor: 'pointer' }}>
            Готово
          </button>
        </div>
  ...
  const Steps = () => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      {needs.map((n, i) => (
        <button key={n.id} onClick={() => setIdx(i)} aria-label={n.chartLabel} style={{
  ...
  const Topbar = () => (
    <div style={{ height: 52, padding: '0 24px', display: 'flex', alignItems: 'center',
      ...
      <div style={{ display: 'flex', gap: 2 }}>
        {onOpenNote && (
          <button onClick={onOpenNote} aria-label="Заметка" style={{ width: 30, height: 30, borderRadius: 6,
        ...
        {onOpenHistory && (
          <button onClick={() => { onOpenHistory(); goBack(); }} aria-label="История" style={{ width: 30, height: 30, borderRadius: 6,
            display: 'grid', placeItems: 'center', background: 'none', border: 'none',
            color: 'var(--text-faint)', cursor: 'pointer' }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  const Footer = () => (
    <div style={{ padding: '14px 24px', display: 'grid', gridTemplateColumns: '1fr auto 1fr',
      alignItems: 'center', borderTop: '1px solid rgba(var(--fg-rgb),0.07)', flexShrink: 0, gap: 8 }}>
      {/* Left: previous need */}
      <div>
        <button onClick={() => idx > 0 && setIdx(idx - 1)} disabled={idx === 0}
          style={{ display: 'inline-flex', alignItems: 'baseline', gap: 8, background: 'none',
            ...
            color: idx === 0 ? 'transparent' : 'var(--text-faint)', fontSize: 14 }}>
          <span>←</span>
      ...
      <div style={{ fontSize: 11, color: 'var(--text-ghost)', letterSpacing: '0.08em',
        textTransform: 'uppercase', fontWeight: 600, textAlign: 'center', whiteSpace: 'nowrap' }}>
        {lastSavedAt
        ...
        {allRated ? (
          <button onClick={() => setShowCompletion(true)}
            style={{ padding: '10px 20px', borderRadius: 8, border: 'none', background: 'var(--text)',
              color: 'var(--bg)', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
            ✓ Готово
          </button>
        ) : nextNeed ? (
          ...
          <button onClick={() => { const f = needs.findIndex(n => !(effectiveRatings[n.id] ?? 0)); if (f >= 0) setIdx(f); }}
            style={{ padding: '10px 20px', borderRadius: 8, border: '1px solid rgba(var(--fg-rgb),0.12)',
              background: 'transparent', color: 'var(--text-sub)', cursor: 'pointer', fontSize: 13 }}>
            К незаполненным ↩
          </button>
        )}
      </div>
