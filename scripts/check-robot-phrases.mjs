...
export const ALLOW = [
  ...
  // «Я психолог — подать заявку»: кнопка, которой СПЕЦИАЛИСТ заявляет о своей
  // квалификации, чтобы получить роль терапевта (webapp/src/pages/account/
  // TherapistRequestSection.tsx и двойник в мини-аппе). Это чужая реплика о
  // себе, а не самоназвание автора — паттерн samonazvanie-avtora её задевать
  // не должен. Контрольный образец в robot-phrases.spec.ts проверяет, что
  // похожая, но НЕзаконная «Я психолог, работаю в подходе…» по-прежнему
  // краснеет.
  /[Яя]\s+психолог\s+—\s+подать\s+заявку/,
  // Та же кнопка, названная в кавычках из другого места («Настройки → "Я
  // психолог"» в боте и в 410-ответе become-therapist): это ссылка на элемент
  // интерфейса, а не утверждение о чьей-то квалификации. Кавычки обязательны —
  // без них запись погасила бы и самоназвание автора.
  /[«"„]Я\s+психолог[»"“]/,
];
...
export const PATTERNS = [
  ...
  // Самоназвание автора (свип 2026-09, docs/VOICE.md). Владелец проекта
  // РАБОТАЕТ В ПОДХОДЕ схема-терапия — и не называет себя ни «схема-
  // терапевтом» (подход — не квалификация, такой специальности нет), ни
  // «психологом» (диплома психолога у него нет, это заявка на квалификацию).
  // Слово «схема-терапевт» расползлось по всему продукту — лендинг, FAQ,
  // тест, карточка терапевта, вывеска в игре, статья, — то есть правило без
  // гейта приезжает обратно с первым же новым текстом.
  //
  // Сам метод не запрещён: «схема-терапия» паттерн не ловит, ловится только
  // «…терапевт». Второй паттерн узкий — он про АВТОРА: «психолог» о других
  // (кнопка заявки, роль в кабинете, Джеффри Янг, «не замена психологу»)
  // законен и под правило не попадает.
  ['samonazvanie-terapevt', /схема-?терапевт/gi],
  [
    'samonazvanie-avtora',
    /(?:Котляревск|Григори|автор)[^.!?]{0,40}психолог|[Яя]\s+психолог/gi,
  ],
];
