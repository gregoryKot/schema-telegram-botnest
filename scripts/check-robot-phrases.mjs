...
export const PATTERNS = [
  ...
  // Самоназвание «схема-терапевт» (свип 2026-09, docs/VOICE.md). Владелец
  // проекта — психолог, который РАБОТАЕТ В ПОДХОДЕ схема-терапия; «схема-
  // терапевт» как профессия звучит как отдельная специальность, которой нет.
  // Слово расползлось по всему продукту (лендинг, FAQ, тест, карточка
  // терапевта в приложении, вывеска в игре, статья) — правило без гейта
  // приезжает обратно с первым же новым текстом. Сам метод не запрещён:
  // «схема-терапия» паттерн не ловит, ловится только «…терапевт».
