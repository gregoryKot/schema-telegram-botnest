...
//
// У храповика есть правильная семантика слияния, и она однозначна. Решение
// принимается для КАЖДОГО ключа отдельно и всегда трёхстороннее:
//   ключ изменила одна сторона — берём её значение как есть (вторая это место
//     не трогала, спорить не о чем; почему это важно — см. merge3);
//   ключ изменили ОБЕ — вот здесь и работает храповик:
//     ratchet-min — метрика, где МЕНЬШЕ лучше (строки файлов, робофразы,
//                   мужской род, дубли, слабые ассерты): берём min;
//     ratchet-max — метрика, где БОЛЬШЕ лучше (покрытие, mutation score):
//                   берём max.
// Так слияние сохраняет прогресс обеих сторон, не даёт случайно ослабить
// планку и не выдаёт за прогресс чужое старое значение.
//
...
function merge3(o, a, b, path) {
  ...
  // Лист. Сначала — трёхсторонний вопрос: а обе ли стороны вообще трогали это
  // место? Сторона, совпадающая с предком, его не трогала, и правку второй
  // берём как есть — какой бы она ни была по величине.
  //
  // Это не деталь, а главное свойство драйвера (инцидент 2026-09-16, PR #496):
  // ветка отведена от старого main, за это время main принял законный рост
  // десятка файлов через `--update` (api.module.ts 59→62, merge.service.ts
  // 306→342…). Ветка этих файлов не видела, но её значения были МЕНЬШЕ — и
  // «всегда min» откатывал бы планку к размерам, которых на слитом дереве уже
  // нет. Гейт после такого слияния краснеет на чужие файлы, а разрешать это
  // приходится руками — ровно та работа, ради отмены которой драйвер и писан.
  if (same(a, o)) return b;
  if (same(b, o)) return a;

  // Обе стороны сдвинули значение относительно предка — вот теперь работает
  // храповик: берём лучшее, чтобы сохранить прогресс обеих.
  if (typeof a === 'number' && typeof b === 'number') return pick(a, b);

  // Не числа (напр. причина-строка в dead-files-baseline) и обе стороны
  // написали своё: храповой семантики тут нет, выдумывать нечего.
