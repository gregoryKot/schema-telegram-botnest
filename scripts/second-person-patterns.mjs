...
export const TY_IMPERATIVES = [
  ...
  ['Начинай', false],
  ['Читай', false],
  // Свип 2026-08-17: «добавь» в пустых состояниях практик нашёлся ручной
  // вычиткой — гейт этого глагола не знал. Заодно соседи того же ряда.
  ['Добавь', false],
  ['Добавляй', false],
  ['Убери', false],
  ['Отложи', false],
...
export const EXCLUDE = [
  ...
  // Единственный владелец-only текст файла — adminPlainText(): заявка,
  // уходящая в личку/на почту ADMIN_ID. По смыслу это то же, что
  // notifyAdminText в CALL_ARG_EXEMPT, но текст собирается в приватном
  // методе, а отправляется из другого места, поэтому по аргументам вызова
  // его не опознать. Второй метод файла (notifyApplicant) пишет
  // пользователю и идёт через t() — на него исключение не влияет.
  /^src\/therapy\/therapist-request\.notify\.ts$/,
  // Отчёт о публикации в канал — уходит владельцу в личку (ADMIN_ID) при
  // сбое: «добавь фразы в админке» адресовано ему, а не подписчику.
